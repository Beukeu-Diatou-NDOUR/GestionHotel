...
def ajouter():
    ...
    date_sorti = entrerDate_sorti.get()
    date_reser = entrerDate_reser.get()
    # Creeon la connexion
    con = sqlite3.connect('hotel.db')
    cuser = con.cursor()
    cuser.execute("insert into client('nom','prenom','tel','num_chambre','date_entre','date_sorti','date_reser') values (?,?,?,?,?,?,?)",(nom,prenom,tel,num_chambre,date_entre,date_sorti,date_reser))
    con.commit()
    con.close()
    messagebox.showinfo("Reservation ajouté")
    # afficher
    con = sqlite3.connect('hotel.db')
    cuser = con.cursor()
    select = cuser.execute("select * from reservation order by code desc")
    select = list(select)
...
def modifier():
    ...
    con = sqlite3.connect('hotel.db')
    cuser = con.cursor()
    cuser.execute("update client set nom=?,prenom=?,tel=?,num_chambre=?,date_entre=?,date_sorti=?,date_reser=?"
                  (nom, prenom, tel, num_chambre, date_entre, date_sorti, date_reser))
    con.commit()
    con.close()
    messagebox.showinfo("Reservation Modifier")
        # afficher
    con = sqlite3.connect('hotel.db')
    cuser = con.cursor()
    select = cuser.execute("select * from reservation order by code desc")
...
def supprimer():
    ...
    con = sqlite3.connect("hotel.db")
    cuser = con.cursor()
    delete = cuser.execute("delete from reservation where code = {}".format(codeSelectionner))
    con.commit()
    table.delete(table.selection())

#titre general
root = Tk()
root.title("Gestion des reservations ")
root.geometry("1300x700")
#Ajouter le titre
lbltitre = Label(root,bd = 20, relief = RIDGE, text="GESTION DES RESERVATIONS", font = ("Arial", 10), bg = "darkblue", fg="white")
lbltitre.place(x = 0, y = 0, width = 1200)
#Liste des patients
lblListePatient = Label(root, text="LISTES DES RESERVATIONS ", font=("Arial", 10), bg="darkblue", fg="white")
...
#entre----------
lblDate_entre = Label(root, text="Date Entré", font = ("Arial", 10), bg = "black", fg="white")
lblDate_entre.place(x=0,y=400,width=200)
entrerDate_entre = Entry(root)
entrerDate_entre.place(x=200,y=400,width="100",height=20)
#sortie ----------
lblDate_sorti = Label(root, text="Date Sorti", font = ("Arial", 10), bg = "black", fg="white")
lblDate_sorti.place(x=0,y=450,width=200)
entrerDate_sorti = Entry(root)
entrerDate_sorti.place(x=200,y=450,width="100",height=20)
...
table.heading(6 , text="DATE_ENTRE")
table.heading(7 , text="DATE_SORTI")
